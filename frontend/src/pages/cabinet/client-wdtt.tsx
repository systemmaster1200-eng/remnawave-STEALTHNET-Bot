...
import { Wifi, Calendar, CreditCard, Loader2, Copy, Check, ChevronDown, Wallet, Shield, Zap, ArrowLeft } from "lucide-react";
import { useClientAuth } from "@/contexts/client-auth";
import { api, type WdttClientCategoryItem, type WdttClientSlotItem } from "@/lib/api";
...
type WdttTariff = WdttClientCategoryItem["tariffs"][number];
...
export function ClientWdttPage() {
  ...
  const [categories, setCategories] = useState<WdttClientCategoryItem[]>([]);
  ...
  const [payModal, setPayModal] = useState<WdttTariff | null>(null);
  ...
  useEffect(() => {
    api.getWdttClientTariffs(token!).then((r) => {
  ...
  async function payByBalance(tariff: WdttTariff) {
    ...
    setPayLoading(true);
    try {
      const res = await api.clientPayByBalance(token, { wdttTariffId: tariff.id } as Parameters<typeof api.clientPayByBalance>[1]);
      setPayModal(null);
      alert(res.message);
      await refreshProfile();
      const r = await api.getWdttSlots(token);
      setSlots(r.items ?? []);
    } catch (e) {
  ...
  async function startYoomoneyPayment(tariff: WdttTariff) {
    ...
    try {
      ...
      } as Parameters<typeof api.yoomoneyCreateFormPayment>[1]);
  ...
  async function startYookassaPayment(tariff: WdttTariff) {
    if (!token || tariff.currency.toUpperCase() !== "RUB") return;
    setPayError(null);
    setPayLoading(true);
    try {
      const res = await api.yookassaCreatePayment(token, {
        ...
        currency: "RUB",
        wdttTariffId: tariff.id,
      } as Parameters<typeof api.yookassaCreatePayment>[1]);
  ...
  async function startCryptopayPayment(tariff: WdttTariff) {
    if (!token) return;
    setPayError(null);
    setPayLoading(true);
    try {
      const res = await api.cryptopayCreatePayment(token, {
        ...
        wdttTariffId: tariff.id,
      } as Parameters<typeof api.cryptopayCreatePayment>[1]);
  ...
  async function startHeleketPayment(tariff: WdttTariff) {
    if (!token) return;
    setPayError(null);
    setPayLoading(true);
    try {
      const res = await api.heleketCreatePayment(token, {
        ...
        currency: tariff.currency,
        wdttTariffId: tariff.id,
      } as Parameters<typeof api.heleketCreatePayment>[1]);
  ...
  async function startLavaPayment(tariff: WdttTariff) {
    if (!token) return;
    setPayError(null);
    setPayLoading(true);
    try {
      const res = await api.lavaCreatePayment(token, {
        ...
        currency: tariff.currency,
        wdttTariffId: tariff.id,
      } as Parameters<typeof api.lavaCreatePayment>[1]);
  ...
  async function startOverpayPayment(tariff: WdttTariff) {
    if (!token) return;
    setPayError(null);
    setPayLoading(true);
    try {
      const res = await api.overpayCreatePayment(token, {
        ...
        currency: tariff.currency,
        wdttTariffId: tariff.id,
      } as Parameters<typeof api.overpayCreatePayment>[1]);
  ...
  async function startPlategaPayment(tariff: WdttTariff, methodId: number) {
    if (!token) return;
    setPayError(null);
    setPayLoading(true);
    try {
      const res = await api.clientCreatePlategaPayment(token, {
        ...
        wdttTariffId: tariff.id,
      } as Parameters<typeof api.clientCreatePlategaPayment>[1]);
          ...
          {isMobileOrMiniapp && (
            <div className="mt-4 pt-4 border-t border-white/5 grid grid-cols-2 gap-3 relative z-10">
              <div className="bg-background/40 rounded-2xl p-3 border border-white/5">
                <p className="text-xs text-muted-foreground font-bold uppercase tracking-wider mb-1">Подключения</p>
          ...
          >
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <TabsContent value="tariffs" className="mt-4">
                ...
                ) : categories.length === 0 || categories.every((c) => c.tariffs.length === 0) ? (
                                  ...
                                  <CardContent className="flex flex-row items-center gap-4 py-4 px-4 min-h-0 min-w-0">
                                    <div className="flex-1 min-w-0 space-y-1.5">
                                      ...
                                      <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px] font-bold uppercase tracking-wider text-muted-foreground">
                                        ...
                                        {t.trafficLimitBytes && (
                                          <span className="flex items-center gap-1.5 bg-background/50 px-2 py-1 rounded-md border border-border/50">
                                            {formatBytes(t.trafficLimitBytes)}
                                          </span>
                                        )}
                                      </div>
                                    </div>
                              ...
                              <CardContent className="flex-1 flex flex-col p-5 min-h-0 min-w-0">
                                ...
                                <div className="flex flex-col gap-2.5 mt-auto mb-5 text-sm font-semibold text-muted-foreground">
                                  ...
                                  <div className="flex items-center gap-3 bg-background/50 px-3 py-2 rounded-xl border border-border/50">
                                    ...
                                    <span>{t.proxyCount} подключений</span>
                                  </div>
                                  {t.trafficLimitBytes && (
                                    <div className="flex items-center gap-3 bg-background/50 px-3 py-2 rounded-xl border border-border/50">
                                      <div className="bg-primary/20 p-1.5 rounded-lg text-primary">
                                      ...
                                      <span>Трафик: {formatBytes(t.trafficLimitBytes)}</span>
                                    </div>
                                  )}
                                </div>
                ...
                {slotsLoading ? (
                  ...
                  <div className="space-y-6">
                    {activeSlots.length > 0 && (
                      <div>
                        <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-wider mb-3 px-1">Активные ({activeSlots.length})</h3>
                        <div className="grid gap-4 sm:grid-cols-2">
                          {activeSlots.map((s) => (
                            <Card key={s.id} className="rounded-3xl border border-border/50 bg-card/40 backdrop-blur-xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden group">
                              <CardContent className="p-5 space-y-4">
                                <div className="flex items-start justify-between gap-2 border-b border-border/50 pb-4">
                                  ...
                                  <div className="flex flex-col items-end text-sm shrink-0">
                                    ...
                                    <span className="font-medium text-foreground">{formatDate(s.expiresAt)}</span>
                                  </div>
                                </div>

                                <div className="space-y-3">
                                  <div className="flex items-center gap-2 min-w-0 bg-background/50 p-2 rounded-2xl border border-border/50">
                                    <div className="pl-2 font-semibold text-xs text-muted-foreground w-14 shrink-0">Ссылка</div>
                                    ...
                                    <Button
                                      ...
                                      onClick={() => copyToClipboard(s.wdttLink, s.id)}
                                    >
                    ...
                    {expiredSlots.length > 0 && (
                      <div>
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                          {expiredSlots.map((s) => (
                            <Card key={s.id} className="rounded-3xl border border-border/50 bg-card/40 backdrop-blur-xl shadow-sm opacity-60">
                              <CardContent className="p-4">
                                <div className="space-y-2">
                                  <div className="flex items-center gap-2">
                                    <span className="text-xs font-mono truncate">{s.wdttLink}</span>
                                    <span className="text-xs text-muted-foreground shrink-0">
                                      ({s.status === "EXPIRED" ? "Истёк" : "Отозван"})
                                    </span>
                                  </div>
